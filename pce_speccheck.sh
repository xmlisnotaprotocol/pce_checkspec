...
function VARIOUS_CHECK() {
    ...
    # Illumio specific systemd for RHEL/CentOS 7
    if [[ "${OSrelease}" == '7' ]]; then
...
function TABLE_OUTPUT() {
    ...
    printf "$width" "OS Version" ${OSrelease}.${OSminor} '6.8~6.10 | 7.4~7.8' "${os_status}"
    ...
    # for CentOS/RHEL 7 systemd
    if [[ "${OSrelease}" == '7' ]] && ([[ "${choose}" == '1' ]] || [[ "${subchoose}" == '1' ]] || [[ "${subchoose}" == '2' ]]); then
...
function PCE_PACKAGES() {
    ...
    echo $line
    if [[ "${OS}" == 'RedHat' || "${OS}" == 'CentOS' ]] && [[ "${OSrelease}" == '7' ]]; then
